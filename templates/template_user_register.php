...
foreach ($data as $key => $value) {
	if(!empty($data[$key]["key"])){
		if(($data[$key]["key"] == "password") || ($data[$key]["key"] == "password2") || ($data[$key]["key"] == "active")){
			unset($data[$key]);
		}else if($data[$key]["key"] == "country"){
			$query = "SELECT * FROM countries WHERE id=\"" . $data[$key]["value"] . "\"";
	...
	<td width="586" align="center" style="text-align:center;">
		<div class="content">';
			...
			{		
				if(!empty($data[$key]["key"])){
					$email_table .= '<tr>';
						$email_table .= '<th>'.(!empty($lg[$data[$key]["key"]]) ? $lg[$data[$key]["key"]] : $data[$key]["key"]).': </th>';
						$email_table .= '<td> '.$data[$key]["value"].'</td>';
					$email_table .= '</tr>';	
				}	
			}
			$email_table .= '</table>';
