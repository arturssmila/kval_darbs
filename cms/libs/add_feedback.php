...
$teaser = !empty($_POST["teaser"]) ? $_POST["teaser"] : "";

require('../../config.php');

$name = mysql_real_escape_string(strip_tags($content));
$content = "<p>".$name."</p>";
$teaser = mysql_real_escape_string(strip_tags($teaser));
...
{
	$template_id = template("S",array("template"=>"feedback"),$tpl) ? $tpl[0]["id"] : 0;
	...
	$query = "INSERT INTO ".PREFIX."meta SET 
		...
		hide_link = '".(!empty($settings["feedback_approoving"])?'1':'0')."'
		";
	mysql_query($query);
	$id =  mysql_insert_id();
	...
	foreach($languages as $key => $val)
	{
		...
		$insert_data[$val["id"]]["teaser"] = $teaser;
