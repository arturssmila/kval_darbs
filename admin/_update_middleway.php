...
if(!empty($url) && ($url == "xml")) //XML update
{
	...
	/**********************************************/
	$zip = new ZipArchive;
	if ($zip->open('../_STORAGE/update.zip') === TRUE)
	{
				...
				if($ff != '.' && $ff != '..')
				{
					...
					else
					{
						if (!(strpos($ff,'~') !== false) && !(strpos($ff,'#') !== false)) {
							...
							*/
							$zip->addFile(str_replace($root, '', $dir.'/'.$ff), str_replace($root, '', $dir.'/'.$ff));
						}
		...
		foreach($update_files as $val)
		{
			if (file_exists(dirname(dirname(__FILE__)).'/'.$val))
			{
				if (!(strpos($val,'~') !== false)) {
					...
					$zip->addFile(str_replace($root, '', dirname(dirname(__FILE__)).'/'.$val), str_replace($root, '', dirname(dirname(__FILE__)).'/'.$val));
				}
			}
		}
		/********************/
	...
	//file_put_contents("../_STORAGE/update.zip",$xml->asXML());
	echo "OK";
	exit();
}
/********************************************************************************************/
